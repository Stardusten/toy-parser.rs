...
impl FiniteAutomaton {
    ...
    /// 添加初态
    fn add_initial_states<'a, I>(&mut self, initial_states: I)
    ...
    /// 添加终态
    fn add_finite_states<'a, I>(&mut self, finite_states: I)
        where I: Iterator<Item = &'a str> {
    ...
    /// 添加一条转换规则
    fn add_transfer_rule(&mut self, from_state_id: &str, input_str: &str, to_state_id: &str) {
    ...
    fn get_all_states_iter(&self) -> impl Iterator<Item = &State> {
        self.adjacency_matrix.keys()
    }

    /// 返回一个包含当前有限状态机中所有状态的 `IntoIterator`
    fn get_all_states_into_iter(&self) -> impl IntoIterator<Item = State> + '_ {
        self.get_all_states_iter().map(|s| s.to_owned())
    }

    /// 返回一个包含当前有限状态机中所有终态的 `Iterator`
    fn get_all_finite_states_iter(&self) -> impl Iterator<Item = &State> {
        self.finite_states.iter()
    }

    /// 返回一个包含当前有限状态机中所有终态的 `IntoIterator`
    fn get_all_finite_states_into_iter(&self) -> impl IntoIterator<Item = State> + '_ {
        self.get_all_finite_states_iter().map(|s| s.to_owned())
    }

    /// 返回一个包含当前有限状态机中所有非终态的 `Iterator`
    fn get_all_infinite_states_iter(&self) -> impl Iterator<Item = &State> {
        self.get_all_states_iter()
            .filter(|s| !self.finite_states.contains(s))
    }

    /// 返回一个包含当前有限状态机中所有非终态的 `IntoIterator`
    fn get_all_infinite_states_into_iter(&self) -> impl IntoIterator<Item = State> + '_ {
        self.get_all_infinite_states_iter().map(|s| s.to_owned())
    }
    ...
    fn calc_epsilon_closure_matrix(&mut self) {
    ...
    /// 注意：调用此方法前，需要先调用 [`calc_epsilon_closure_matrix`] 计算 ɛ 闭包矩阵
    fn get_epsilon_closure<'a, I>(&self, query_states: I) -> Option<BTreeSet<State>>
        where I: Iterator<Item = &'a State> {
        self.epsilon_closure_matrix.as_ref().map(|epsilon_closure_matrix| {
            query_states
...
impl Debug for FiniteAutomaton {
